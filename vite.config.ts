...
import { writeDefAesConfig } from './scripts/writeDefAesConfig';
import { createObfuscator } from './scripts/obfuscator';

// https://vitejs.dev/config/
export default ({ command, mode }) => {
  writeDefAesConfig();
  // 解决 uniapp 打包时强制覆盖 NODE_ENV 和 VITE_USER_NODE_ENV 为 production 的问题
  if (mode === 'test') {
  ...
  return defineConfig({
    ...
    server: {
      host: '0.0.0.0',
      hmr: true,
      port: Number.parseInt(VITE_APP_PORT, 10), // 仅 H5 端生效，其他端不生效（其他端走build，不走devServer)
      proxy: JSON.parse(VITE_APP_PROXY)
        ? {
            [VITE_APP_PROXY_PREFIX]: {
              ...
              rewrite: (path) => {
                console.log(path);
                return path.replace(new RegExp(`^${VITE_APP_PROXY_PREFIX}`), VITE_SERVER_API);
              }
            }
          }
        : undefined
